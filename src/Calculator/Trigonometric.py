...
class Trigonometric:
    def __init__(self,master,disp):
        group = LabelFrame(master, text="Trigonometric", padx=34, pady=7)
        group.place(x=350,y=100)

        self.val = disp.textin
        #w = Label(group,text="Trigonometric")
        self.SineBut=Button(group,padx=15,pady=5,bd=4,bg='white',command=lambda:self.Sine(self.val.get()),text="Sin",font=("Courier New",16,'bold'))
        self.SineBut.pack()
        self.CosBut=Button(group,padx=15,pady=5,bd=4,bg='white',command=lambda:self.Cos(self.val.get()),text="Cos",font=("Courier New",16,'bold'))
        self.CosBut.pack()
